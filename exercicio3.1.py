...
#criação de classe de publicação
@dataclass
class Publicacao:
    conteudo: str
    descricao: str
...
#Aqui estamos criando uma publicação
...
#Aqui estou criando a opção de curtida na publicação    
def curtir_publicacao():
    ...
    visualizar_feed()
    try:
        indice = int(input("Digite o número da pulicação para curtir: ")) - 1
...
#Aqui estou desenvolvendo opção de visualização       
def visualizar_feed():
    print("\n=== FEED ===")
    if not lista_publicacoes:
        print("Nenhuma publicação disponível.")
...
# Aqui estou criando o sistema de visualizar publicação avulso       
...
# Aqui será definido o tipo de visualização por autor    
...
# Criação do menu para ser mostrado.    
def menu():
    while True:
        print("\n REDE SOCIAL ===")
